...
echo "<center><table><tr>";
    ...
    $Sort = mysql_real_escape_string(strip_tags(stripslashes($_GET['Sort'])));
      ...
      if (!$s) {
        ...
        $getItems = mysql_query("SELECT * FROM UserStore WHERE Active='0' AND itemDeleted='0' ORDER BY ID DESC LIMIT {$Minimum},  ". $Setting["PerPage"]);
      
      }
      elseif ($s) {
      
        $getItems = mysql_query("SELECT * FROM UserStore  WHERE Name LIKE '%$q%' AND Active='0' AND itemDeleted='0' ORDER BY ID DESC");
      
      }
      }
      else {
      $getItems = mysql_query("SELECT * FROM UserStore WHERE Active='0' AND itemDeleted='0' AND Type='$Sort' ORDER BY ID DESC LIMIT {$Minimum},  ". $Setting["PerPage"]);
      }
