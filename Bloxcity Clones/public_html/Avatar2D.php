...
//load up database
$connection = mysql_connect("mysql.ct8.pl","m27001_social","Sg090228") or die("We are currently overloaded with users. Please try again in 1-5 minutes. Do not constantly refresh the page.");
mysql_select_db("m27001_social") or die("We are currently overloaded with users. Please try again in 1-5 minutes. Do not constantly refresh the page.");
...
//if the user is online, show online symbol
	...
	$Image->AddLayer("Online.png");
	}
//or else if the user is offline, show offline symbol
	else {
	$Image->AddLayer("Offline.png");
	}
//if the user is premium, show a little premium icon to the bottom left of their avatar
	if ($gU->Premium == 1) {
	$Image->AddLayer("pw.png");
	}
//if the user is owner, show a little owner icon to the bottom left of their avatar
	if ($gU->Owner == 1) {
	$Image->AddLayer("owner.png");
	}
//if the user is owner, show a little owner icon to the bottom left of their avatar
	if ($gU->Admin == 1) {
	$Image->AddLayer("admin.png");
	}
