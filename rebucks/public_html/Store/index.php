...
<tr>
<td width="200">
<div style="border: 1px solid black; color: black; background-color: white; padding: 8px; vertical-align: text-top;">
<?php
if ($logged == true){
		?>
		<b><font color='red'>Collectibles only are displayed in the Collectibles tab.</font></b><br />
		<fieldset>
...
<a href="?Type=1" id="buttonsmall2">Collectibles</a><br /><br>
...
<a href="?Sort=Mouth&Type=<?php print_r($type); ?>">Mouths</a><br>
...
for ($count = 1; $count <= $cc; $count ++){
	...
	$o ++;
	?>
    <td width="100" height="240" style="max-width: 100px; padding: 3px; border: 1px solid black; background-color: white; color: black; text-align: center; ">
    <a title="<?php print_r($cr['Name']); ?>" href="Item.php?id=<?php print_r($cr['id']); ?>">
	...
	if ($cr['islimited'] != 0){
		?>
        <img src="/assets/Limited.png" width="100" height="195" style="" />
        <?php
	}
	?>
    <br />
    ...
    <?php
	if ($cr['price'] == '0'){
		print_r("<font color='#FF9900'>" . $cr['reebprice'] . " Gold Coins</font>");
	}else{
		print_r("<font color='#00CC00'>" . $cr['price'] . " Bucks</font>");
	}
	?>
    </td>
    <?php
