...
".number_format($Users)."
	...
	$getMembers = mysql_query("SELECT * FROM Users WHERE Username LIKE '%$q%' ORDER BY ID");
		...
		while ($gM = mysql_fetch_object($getMembers)) {
			$counter++;
			echo "<td width='125'><center><a href='/Profile/$gM->Username' border='0'>";if($gC->Avatars == "3D"){echo"<img src='https://avatars.mine2build.eu/$gM->Avatar3D' width='300' height='350'>";}elseif($gC->Avatars == "2D"){echo"<img src='/Avatar2D.php?ID=$gM->ID'>";};echo"<br /><smalltextlink>$gM->Username</smalltextlink></a></td>";
			if ($counter>= 6) {
				echo "</tr><tr>";
				$counter = 0;
			}
				...
				while ($gM = mysql_fetch_object($getMembers)) {
			$counter++;
			echo "<td width='125'><center><a href='/Profile/$gM->Username' border='0'>";if($gC->Avatars == "3D"){echo"<img src='https://avatars.mine2build.eu/$gM->Avatar3D' width='300' height='350'>";}elseif($gC->Avatars == "2D"){echo"<img src='/Avatar2D.php?ID=$gM->ID'>";};echo"<br />$gM->Username</a></td>";
			if ($counter>= 6) {
				echo "</tr><tr>";
				$counter = 0;
			}
