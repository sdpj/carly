<?php
  ...
  $World2BuildTopics = mysql_query("SELECT * FROM Topics WHERE ID='1' OR ID='2' OR ID='3'");
  $ClubHousesTopics = mysql_query("SELECT * FROM Topics WHERE ID='4'");
  $UnrelatedTopics = mysql_query("SELECT * FROM Topics WHERE ID='5' OR ID='6' OR ID='7' OR ID='8' OR ID='9' OR ID='10'");
  $OBFunTopics = mysql_query("SELECT * FROM Topics WHERE ID='11'");
?>
    ...
    <table cellpadding='0' name='Topic1' cellspacing='0' width='100%'>
      <?while($Unrelated = mysql_fetch_object($UnrelatedTopics)){
        ...
        <tr>
          <td width='70%' class='forum-table-row'>
            <div class='topic-name-description' style='width:87%'>
              <a href='/forum/ShowForum.aspx?ID=".$Unrelated->ID."' class='forumTitle'><img src='/forum/forum_status.gif'>                  ".$World2Build->TopicName."
                ".$Unrelated->TopicName."
                <div style='margin-top:5px;'></div>
                <font style='font-size:13px;font-weight:normal;'>
