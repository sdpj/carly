<?
  ...
  //query
  $allusers = mysql_query("SELECT * FROM UserStore WHERE itemDeleted='0' ORDER BY ID DESC");
  ...
  $StoreEnabled = mysql_query("SELECT * FROM StoreStatus");
  $StoreOnlineOffline = mysql_fetch_object($StoreEnabled);
  if (!$SearchSubmit && !$MinPrice && !$MaxPrice) {
    $getItems = mysql_query("SELECT * FROM UserStore WHERE itemDeleted='0' ORDER BY ID DESC LIMIT {$Minimum},  ". $Setting["PerPage"]);
  }
  elseif ($SearchSubmit){
    $getItems = mysql_query("SELECT * FROM UserStore WHERE Name LIKE '%".$SearchQuery."%' AND itemDeleted='0' ORDER BY ID DESC");
  }
  if ($Sort){
    $getItems = mysql_query("SELECT * FROM UserStore WHERE Name LIKE '%".$SearchQuery."%' AND itemDeleted='0' AND Type='".$Sort."' ORDER BY ID DESC");
  }
  if($Sort == "Limiteds"){
    $getItems = mysql_query("SELECT * FROM UserStore WHERE Name LIKE '%".$SearchQuery."%' AND itemDeleted='0' AND saletype='limited' ORDER BY ID DESC");
  }
...
?>
  <div class='catalog-page'>
    <h1 style='float:left;'>User Catalog</h1>
    ...
    <div class='divider-right' style='width:175px;float:left;'>
      ...
      <div class='divider-bottom' style='position: relative;z-index:3;'>
        <div class='catalog-legend-sub' style='margin-top:10px;font-size:11px;padding:5px;'>
          ...
          <b style='font-size:12px;'>BC Item</b>
          <br />
          An item that is only available to members with a Builder's Club membership.
        </div>
      </div>
    </div>
    <div style='float:left;width:80%;margin-left:10px;'>
      ...
      <h1 style='margin:0;padding:0;font-weight:normal;'>
        ...
        <div style='float:right;'>
          ...
          if($User) { echo "
          <a href='UserUpload.aspx' class='btn-primary'>
            Upload Item
          </a>"; }
          echo"
        </div>
      ...
      ";
          while($gI = mysql_fetch_object($getItems)){
            ...
            if ($gI->saletype == "limited") {
              echo"
              <a href='/Catalog/UserItem.aspx?ID=".$gI->ID."'>
                <div style='width:100px;border:1px solid #ccc;padding:3px;'>
                  <div style='width: 100px; height: 200px; z-index: 3;  background-image: url(/Catalog/Dir/Avatar.png);'>
                  <div style='width: 100px; height: 200px; z-index: 3;  background-image: url(/Catalog/Dir/".$gI->File.");'>
              ...
              ";
                ...
                <a href='/Catalog/UserItem.aspx?ID=".$gI->ID."'>
                <div style='width:100px;border:1px solid #ccc;padding:3px;'>
                  <div style='width: 100px; height: 200px; z-index: 3;  background-image: url(/Catalog/Dir/image_denied.png);'>
                  </div>
                </div>
              </a>
              ";
                ...
                <a href='/Catalog/UserItem.aspx?ID=".$gI->ID."'>
                <div style='width:100px;border:1px solid #ccc;padding:3px;'>
                  <div style='width: 100px; height: 200px; z-index: 3;  background-image: url(/Catalog/Dir/pending_moderation.png);'>
                  </div>
                </div>
              </a>
              ";
            ...
            <div style='margin-top:3px;height:auto;width:100px;border:1px solid #ccc;padding:3px;'>
              <a href='/Catalog/UserItem.aspx?ID=".$gI->ID."' title='".$gI->Name."'>
                <b style='font-size:13px;'>".$gIName."</b>
              </a>
              <br />
              <font style='color:gray;font-size:11px;'>
                <b>By:</b>
                <a href='/user.aspx?ID=".$gC->ID."' style='font-size:11px;'>
                  ".$gC->Username."
                </a>
              </font>
              <br />
