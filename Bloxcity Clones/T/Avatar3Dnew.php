<?php //header("Content-Type: image/png");
  //prototype wip thing for real 3d in sans, which loads the actual items your character is wearing when you input a Username in the url, so far, it loads the face and t-shirt
  
  @mysql_connect("mysql.ct8.pl","m24817_mobix","Sg090228");
    mysql_select_db("m24817_mobix");
...
?>
<?php
$Wallpaper = "/Store/Dir/".$gU->Background;    
$FACE = "/Store/Dir/".$gU->Mouth;
$Bottom = $gU->Bottom;
$Tshirt = "/Store/Dir/".$gU->Top;
  if (!$Top) {
$Tshirt = "/Store/Dir/thingTransparent.png";
}
  
  
$Shirt = "/Store/Dir/".$gU->Shirt;
  if ($Shirt = " ") {
$Shirt = "/Store/Dir/ffe0bd.png";
}
  
$EatPant = "/Store/Dir/".$gU->Pants;
  if ($EatPant = " ") {
$EatPant = "/Store/Dir/ffe0bd.png";
}
...
?><style>body { background-image: url("<?php echo $Wallpaper?>"); background-size: 100% 100%; }</style>
<head>
...
$Hat = "thingTransparent";
          ...
          // HEAD
          THREE.ImageUtils.crossOrigin = "";
          var texture =  THREE.ImageUtils.loadTexture("<?php echo $FACE?>");
          var texturetwo =  THREE.ImageUtils.loadTexture("https://dijf55il5e0d1.cloudfront.net/images/na/5/0/2/50230_1000.jpg");
          
          var textureHead = THREE.ImageUtils.loadTexture("/Store/Dir/ffe0bd.png"); //sorry thats theres no skin customization, rip
          var textureLeftArm = THREE.ImageUtils.loadTexture("<?php echo $_GET['la']?>");
          var textureTorso = THREE.ImageUtils.loadTexture("<?php echo $_GET['t']?>");
          var textureRightArm = THREE.ImageUtils.loadTexture("<?php echo $_GET['ra']?>");
          var textureLeftLeg = THREE.ImageUtils.loadTexture("<?php echo $_GET['ll']?>");
          var textureRightLeg = THREE.ImageUtils.loadTexture("<?php echo $_GET['rl']?>");
          ...
          face.position.z = 0.22;
              ...
              // T SHIRT
              var texture =  THREE.ImageUtils.loadTexture("/<?php echo $Tshirt?>");
              ...
              var texture = THREE.ImageUtils.loadTexture("<?php echo $Shirt?>");
              texture.flipY = false;
              texture.flipX = false;
              ...
              var texture = THREE.ImageUtils.loadTexture("<?php echo $EatPant?>");
              texture.flipY = false;
              texture.flipX = false;
              var material = new THREE.MeshPhongMaterial({map: texture, color: 0xffe0bd, transparent: true, opacity:1});
          ...
          render();
            ...
            function checkLoaded() {
              if (headDone == 1 && hatDone == 1) {
                ...
                $.ajax({
                  type: "POST",
                  url: "/saveavatar.php?Username=<?echo"$Username";?>",
                  data: {
                   base: Photo
                  }
                }).done(function(o) {
