...
$width = intval($avatarwidth);
$height = intval($avatarheight);
		if ($validator->fails())
		{
		        return Redirect::to('/store/upload')
		                ->withErrors($validator->messages());
		} elseif (Image::make(Input::file('image'))->width() != $width || Image::make(Input::file('image'))->height() != $height) {
		...
		$inventory->item_id 			= $new_item->id;
		$inventory->save();

		Logging::LogEventWithData('item_purchase', Auth::user()->id, $_SERVER['REMOTE_ADDR'], Auth::user()->currency_1, $new_item->id);
