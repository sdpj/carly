...
<div class="bg-white pt-2 pb-2">
   <div class="row">
      <div class="col-6">
         <p class="fw-bold"><a href="/2016/Forum/"><?=$sitename;?> Forum</a> » <a href="ShowForum.aspx?ForumID=<?=$topic->id;?>"><?=$topic->name;?></a></p>
      </div>
      <div class="col-6">
         <p class="text-end fw-bold"><span><a class="normal" href="/Forum/">Home</a></span><span class="pe-1 ps-1">|</span><span><a class="normal" href="MyForums.aspx">MyForums</a></span></p>
      </div>
   </div>
   <div class="w-100">
      <?php if($user) {?><div class="float-left"><a href="AddPost.aspx?ForumID=<?=$id;?>">New Thread</a></div><?}?>
   </div>
   <table class="w-100">
...
?>
         <tr class="bodyRow-0-2-26">
            <td>
               <a class="normal" href="ShowPost.aspx?PostID=<?=$gT->threadId;?>">
...
<?}?>
   ...
   <div class="row">
	...
	echo '<style>.leftthing{margin-left:5px;}</style><a href="ShowForum.aspx?ForumID='.$id.'&page='.($page-1).'" style="display:inline-block!important;">'.$otherpage1.'</a>';
	}
        echo "<thing class='leftthing rightthing'>".htmlspecialchars($page)."</thing>";
	if ($page < ($amount)) { $otherpage2 = $page + 1;
	echo '<style>.rightthing{margin-right:5px;}</style><a href="ShowForum.aspx?ForumID='.$id.'&page='.($page+1).'" style="display:inline-block!important;">'.$otherpage2.'</a>';
	}
?>
      </span></p>
      </div>
