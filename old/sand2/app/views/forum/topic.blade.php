...
@if( Session::has('delete-success') )
	<div class="well success-well">
		<p>The selected thread has been deleted! <a href="{{ url('/forum/thread/'.Session::get('threadid').'/restore') }}">(undo this action)</a></p>
	</div>
@endif 
@if( Session::has('scrub-success') )
	<div class="well success-well">
		<p>The selected post has been scrubbed!</p>
	</div>
@endif 
@if( Session::has('unscrub-success') )
	<div class="well success-well">
		<p>The selected post has been unscrubbed!</p>
	</div>
@endif 
			...
			@endif
				<div class="forum-thread-inside">
					<div class="pull-left">
						...
						@endif
						@if ($profanity == true)
							<h4>{{{ Filter::filter(str_limit($thread->title, $limit = 50, $end = '...'), '***') }}}</h4>
						@else
							<h4>{{{ str_limit($thread->title, $limit = 50, $end = '...') }}}</h4>
						@endif
					</div>
					<div class="pull-right">
						...
						@if ($thread->thread->last())
							...
							@if ($thread->thread->last()->created_at->diffInDays() > 1)
								...
								@else
									<i>at {{{ $thread->thread->last()->created_at->toDayDateTimeString() }}}</i>
								@endif
							@else
