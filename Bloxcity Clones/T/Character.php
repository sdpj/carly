...
<table width='80%'>
  <tr>
    <td style='width:250px;' valign='top'>
      <div id='ProfileText' style='text-align:center;'>
        <h4>Avatar</h4>
      </div>
        <div id=''>
          <center>
          ";
          if($gC->Avatars == "3D"){
echo"<img src='/Avatars/$myU->Avatar3D' width='256' height='256'><wrong style='float:left;' class='wrong'>Something wrong with your avatar? <a href='/Character.php?Rerender=true'>Click here to re-render it!</a></wrong>";}elseif($gC->Avatars == "2D"){
echo"<img src='/Avatar2D.php?ID=$myU->ID'>";
};
          echo "
        </div>
      <br />
      ...
      <div style='overflow-x:auto;width:0px;max-width:0px;'>
        <table><tr>
        ";
        $counter = 0;
        if (!empty($myU->Background)) {
          ...
          if ($check1 == "1") {
            ...
            $gN = mysql_fetch_object($getName);
            $Link = "/Store/UserStore.php";
          
          }
          
          $getID = mysql_query("SELECT * FROM Inventory WHERE UserID='$myU->ID' AND File='$gN->File'");
          $gI = mysql_fetch_object($getID);
          $echo = "</a><br><a href='?Page=$Page&Sess=".$gI->code1."&Var=".$gI->code2."&Remove=Yes&Rerender=true' style='font-size:10pt;'><font color='red'>Remove</font></a>";
          $echo1 = "<br><a href='?Page=$Page&Sess=".$gI->code1."&Var=".$gI->code2."&Remove=Yes&Rerender=true' style='font-size:10pt;'>";
  $TheItemName = $gN->Name;
              ...
              if($Page&&$Sess&&$Var&&$Wear)
              {
                ...
                else if($gi1 >= 1)
                {
                  ...
                  if($gi->code1 == $code1&&$gi->code2 = $code2)
                  {
                    ...
                    header("Location: /Character.php?Page=$Page");
  ...
  $TheItemName = $TheItemName . "...";
    ...
    /*bottom: 461px;*/vertical-align: top;' valign='top'>
<div id='ProfileText' style='text-align:center;'>
        <h4>Inventory</h4>
      </div>
      <div id=''>
...
if ($Items == 0) {
echo "Looks like you don't have any items! Why not look for some in the <a href='/Store/UserStore.php'>store</a>?";
}
...
$Type = $gI->Type;
  ...
  if($myU->$type == $gI->File)
  {
    ...
    $echo = "</a><br><a href='?Page=$Page&Sess=".$gI->code1."&Var=".$gI->code2."&Remove=Yes&Rerender=true' style='font-size:10pt;'><font color='red'>Remove</font></a>";
    $echo1 = "</a><br><a href='?Page=$Page&Sess=".$gI->code1."&Var=".$gI->code2."&Remove=Yes&Rerender=true' style='font-size:10pt;'>";
  
  }
  else
  {
  
    $echo = "<br><a href='?Page=$Page&Sess=".$gI->code1."&Var=".$gI->code2."&Wear=Yes&Rerender=true' style='font-size:10pt;'><font color='green'>Wear</font></a>";
    $echo1 = "<br><a href='?Page=$Page&Sess=".$gI->code1."&Var=".$gI->code2."&Wear=Yes&Rerender=true' style='font-size:10pt;'>";
  ...
  echo "<td align='center' valign='top' style=''>$echo1<img src='/Store/Dir/thingTransparent.png' width'100 height'100'><br><!--<img src='/Store/Dir/".$gI->File."' width='100' height='200'>--><notice style='float:left;'>Item Renders Are WIP</notice></a><br><a href='".$Link."?ID=".$gI->ItemID."' border='0' title='".$gN->Name."'><br />".$TheItemName."<br><font size='2'>
  ";
  echo "
  ";
...
if ($Page > 1) {
echo '<a href="/Character.php?Page='.($Page-1).'">Previous</a> - ';
}
echo '   Page '.$Page.' of '.(ceil($num / $Setting["PerPage"]));
if ($Page < ($amount)) {
echo ' - <a href="/Character.php?Page='.($Page+1).'">Next</a>';
}
echo "<a name='bottom'></a>";
echo "</div></td></tr></table><br><br>";
}
  if (isset($_GET['Rerender'])) {
    echo "<iframe src='/Avatar3D.php?ID=$myU->ID' style='position: absolute;width:0;height:0;border:0;'></iframe>";
    echo "<meta http-equiv=\"refresh\"
content=\"10;URL=/Character.php\">";
    echo "<script>var arr = document.getElementsByClassName(\"wrong\");
for (var i = 0; i < arr.length; i++) {
   arr[i].innerHTML = \"Rendering... Please wait 10 seconds.\";
}</script>";
}
include "Footer.php";