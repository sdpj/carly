...
pcall(function() game:SetPlaceID(<?=$_GET['place'];?>, true) end)
...
function reportContentProvider(time, queueLength, blocking)
	...
	pcall(function()
		...
		game:HttpGet("http://www.yrk.ct8.pl/Analytics/ContentProvider.ashx?t=" .. time .. "&ql=" .. queueLength, blocking and not isTouchDevice)
...
function reportCdn(blocking)
	...
	pcall(function()
		...
		if successDelta > 0 or failureDelta > 0 then
			...
			game:HttpGet("http://www.yrk.ct8.pl/Game/Cdn.ashx?source=client&success=" .. successDelta .. "&failure=" .. failureDelta, blocking and not isTouchDevice)
...
function reportDuration(category, result, duration, blocking,errorType)
	...
	pcall(function() game:HttpGet("http://www.yrk.ct8.pl/Game/JoinRate.ashx?st=0&i=0&p=-1&c=" .. category .. "&r=" .. result .. "&d=" .. (math.floor(duration*1000)) .. "&b=" .. bytesReceived .. "&ip=localhost&errorType=" .. errorType .. "&platform=" .. platform, blocking and not isTouchDevice) end)
...
local closeConnection = game.Close:connect(function() 
	...
	if 0 then
		...
		if true then pcall(function() game:HttpPost("https://api.yrk.ct8.pl/auth/invalidate", "invalidate") end) end
...
game:GetService("InsertService"):SetBaseSetsUrl("http://www.yrk.ct8.pl/Game/Tools/InsertAsset.ashx?nsets=10&type=base")

game:GetService("InsertService"):SetUserSetsUrl("http://www.yrk.ct8.pl/Game/Tools/InsertAsset.ashx?nsets=20&type=user&userid=%d")

game:GetService("InsertService"):SetCollectionUrl("http://www.yrk.ct8.pl/Game/Tools/InsertAsset.ashx?sid=%d")

game:GetService("InsertService"):SetAssetUrl("http://www.yrk.ct8.pl/Asset/?id=%d")

game:GetService("InsertService"):SetAssetVersionUrl("http://www.yrk.ct8.pl/Asset/?assetversionid=%d")



pcall(function() game:GetService("SocialService"):SetFriendUrl("http://www.yrk.ct8.pl/Game/LuaWebService/HandleSocialRequest.ashx?method=IsFriendsWith&playerid=%d&userid=%d") end)

pcall(function() game:GetService("SocialService"):SetBestFriendUrl("http://www.yrk.ct8.pl/Game/LuaWebService/HandleSocialRequest.ashx?method=IsBestFriendsWith&playerid=%d&userid=%d") end)

pcall(function() game:GetService("SocialService"):SetGroupUrl("http://www.yrk.ct8.pl/Game/LuaWebService/HandleSocialRequest.ashx?method=IsInGroup&playerid=%d&groupid=%d") end)

pcall(function() game:GetService("SocialService"):SetGroupRankUrl("http://www.yrk.ct8.pl/Game/LuaWebService/HandleSocialRequest.ashx?method=GetGroupRank&playerid=%d&groupid=%d") end)

pcall(function() game:GetService("SocialService"):SetGroupRoleUrl("http://www.yrk.ct8.pl/Game/LuaWebService/HandleSocialRequest.ashx?method=GetGroupRole&playerid=%d&groupid=%d") end)

pcall(function() game:GetService("GamePassService"):SetPlayerHasPassUrl("http://www.yrk.ct8.pl/Game/GamePass/GamePassHandler.ashx?Action=HasPass&UserID=%d&PassID=%d") end)

pcall(function() game:GetService("MarketplaceService"):SetProductInfoUrl("https://api.yrk.ct8.pl/marketplace/productinfo?assetId=%d") end)

pcall(function() game:GetService("MarketplaceService"):SetPlayerOwnsAssetUrl("https://api.yrk.ct8.pl/ownership/hasasset?userId=%d&assetId=%d") end)
...
function registerPlay(key)
	...
	if true and game:GetService("CookiesService"):GetCookieValue(key) == "" then
		...
		game:GetService("CookiesService"):SetCookieValue(key, "{ \"userId\" : <?=$usr['id'];?>, \"placeId\" : <?=$_GET['place'];?>, \"os\" : \"" .. settings().Diagnostics.OsPlatform .. "\" }")
...
local success, err = pcall(function()	
	...
	playerConnectSucces, player = pcall(function() return client:PlayerConnect(<?=$usr['id'];?>, "<?=$_GET['ip'];?>", <?=$_GET['port'];?>, 0, threadSleepTime) end)

	if not playerConnectSucces then

		--Old player connection scheme

		player = game:GetService("Players"):CreateLocalPlayer(<?=$usr['id'];?>)

		analytics("Created Player")

		client:Connect("<?=$_GET['ip'];?>", <?=$_GET['port'];?>, 0, threadSleepTime)
	...
	if true then
		...
		pcall(function() game:HttpPost("https://api.yrk.ct8.pl/auth/negotiate?ticket=", "negotiate") end)

		delay(300, function()

			while true do

				pcall(function() game:HttpPost("https://api.yrk.ct8.pl/auth/renew", "renew") end)
	...
	pcall(function() player.Name = [========[<?=$usr['username'];?>]========] end)
...
pcall(function() game:SetVideoInfo('<?xml version="1.0"?><entry xmlns="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://gdata.youtube.com/schemas/2007"><media:group><media:title type="plain"><![CDATA[ROBLOX Place]]></media:title><media:description type="plain"><![CDATA[ For more games visit http://www.yrk.ct8.pl]]></media:description><media:category scheme="http://gdata.youtube.com/schemas/2007/categories.cat">Games</media:category><media:keywords>ROBLOX, video, free game, online virtual world</media:keywords></media:group></entry>') end)
