...
?><?php if ($reply == true){
  $heading = "Replying to ".$userName;
  }else{
    $heading = "Messaging ".$userName;
    }?>
<h1 style="margin:0px;text-align:center;font-weight:normal;"><?php echo $heading;?></h1><br>
<div style="width: 98%; background-color: white; color: black; letter-spacing: 1px; font-weight: 600; box-shadow: 3px 4px 12px 0px rgb(0 0 0 / 15%);
    border-radius: 5px; padding: 5px; text-align: left; vertical-align: text-top;">
<form action="?id=<?php echo($id); ?>" method="post">
<input type="text" name="subject" style="margin:auto;display:block;background-color:#f9f9f9;box-shadow:2px 1px 4px 0px rgb(0 0 0 / 9%);border-radius:3px;border: 0px;outline: none;" <?php if ($reply == false){ ?>placeholder="Subject.."<?php }else{ ?>value="<?php print_r("RE: " . $message['subject']); } ?>" /><br>
<textarea name="body" style="margin: 2px; height: 269px; width: 1001px; margin:auto;display:block;background-color:#f9f9f9;box-shadow:2px 1px 4px 0px rgb(0 0 0 / 9%);border-radius:3px;border: 0px;outline: none;" placeholder="Message here.."><?php /*if ($reply == true){ ?>
...
<input type="submit" value="Send Message" id="buttonsmall" style="border-radius: 5px;
    background-color: #04AA6D;
    transition-duration: 0s; display:block; text-align:center; margin:auto; width: 97%; border: 0px; margin-bottom: 5px;" />
</center>
</form>
</div>
