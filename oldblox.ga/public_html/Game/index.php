...
// do user logout
  ...
  if($_GET['do']=='logout'){
    ...
    $sql = "UPDATE Users SET room = '1', online_time = '0' WHERE Username = '".mysql_real_escape_string($_SESSION['Username'])."'";mysql_query($sql) or die(mysql_error());
    ...
    if($_GET['action']=='banned'){
      ...
      $sql = "UPDATE Users SET status = '1' WHERE Username = '".mysql_real_escape_string($_SESSION['Username'])."'";mysql_query($sql) or die(mysql_error());
  ...
  $down = mysql_query("SELECT * FROM down");
  $down = mysql_fetch_object($down);
  
  if ($down->down == "1") {
  if ($_SERVER['REMOTE_ADDR'] != "68.62.209.0") {
  die("Game area is down for maintenance.");
  }
  }
