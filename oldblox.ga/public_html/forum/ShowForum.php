	...
	<div class='forum-category-page' style='padding-top:10px;'>
		...
		<div style='text-align:left;padding-top:10px;padding-bottom:10px;'>
			<form action='' method='POST' style='margin:0;padding:0;'>
				<a href='/forum/NewPost.aspx?ID=<?echo$Topic->ID;?>' class='btn-primary' style='padding:3px;font-size:13px;'>
					New Thread
				</a>
				<a href='/forum/TopPosters.aspx' class='btn-primary' style='padding:3px;font-size:13px;'>
					Top 10 Posters
				</a>
		...
		<table cellpadding='0' name='Topic1' cellspacing='0' width='100%'>
			<?while($Global = mysql_fetch_object($Globals)){
				...
				if($Global->Locked == "0"){
					$ForumImage = "<img src='/Images/warning.png' style='padding-right:5px;vertical-align:middle;' width='25'>";
				}
				elseif($Global->Locked == "1"){
					$ForumImage = "<img src='/Images/warning.png' style='padding-right:7px;vertical-align:middle;' width='25'>";
				}
				echo"
				<tr>
					<td width='60%' class='forum-table-row' style='max-width:60%;border-right:1px solid #ccc;padding:10px;'>
						<div class='topic-name-description' style='width:87%'>
							".$ForumImage."
							<a href='/forum/ShowPost.aspx?ID=".$Global->ID."'>
				...
				<td class='forum-table-row' width='10%' style='padding-top:10px;padding-bottom:10px;'>
						<a href='/user.aspx?ID=".$gP->ID."' style='font-size:13px;color:#363636;'>
							<center>
								<font style='font-size:10px;'>
		...
		<table cellpadding='0' name='Topic1' cellspacing='0' width='100%'>
			<?while($Sticky = mysql_fetch_object($Stickies)){
				...
				if ($lR == 0) {
					...
					$gP = mysql_fetch_object($getPoster);
					$LastReply = "
					<a href='/user.aspx?ID=".$gP->ID."' style='color:#363636;'>
						<center>
							<font style='font-size:10px;'>
								<b>".$lR->TimePosted."</b>
							</font>
				...
				if($Sticky->Locked == "0"){
					$ForumImage = "<img src='/Images/stickytopic.gif' style='padding-right:5px;vertical-align:middle;'>";
				}
				elseif($Sticky->Locked == "1"){
					$ForumImage = "<img src='/forum/Images/sticky-locked.png' style='padding-right:5px;vertical-align:middle;'>";
				}
				echo"
				<tr>
					<td width='60%' class='forum-table-row' style='max-width:60%;padding:10px;'>
						<div class='topic-name-description' style='width:87%'>
							".$ForumImage."
							<a href='/forum/ShowPost.aspx?ID=".$Sticky->ID."'>
								<font style='font-size:14px;font-weight:normal;color:#363636;vertical-align:middle;'>
		...
		<table cellpadding='0' name='Topic1' cellspacing='0' width='100%'>
			<?while($Thread = mysql_fetch_object($Threads)){
				...
				if ($lR == 0) {
					...
					$gP = mysql_fetch_object($getPoster);
					$LastReply = "
					<a href='/forum/ShowPost.aspx?ID=".$Thread->ID."#".$lR->ID."'>
				...
				if($Thread->Views > 45 && $Thread->Locked == "1"){
					$ForumImage = "<img src='/Images/hotpost-locked.png' style='padding-right:5px;vertical-align:middle;'>";
				}
				elseif($Thread->Views > 45 && $Thread->Locked == "0"){
					$ForumImage = "<img src='/Images/hotpost.png' style='padding-right:5px;vertical-align:middle;'>";
				}
				elseif($Thread->Locked == "0"){
					$ForumImage = "<img src='/Images/regpost.gif' style='padding-right:5px;vertical-align:middle;'>";
				}
				elseif($Thread->Locked == "1"){
					$ForumImage = "<img src='/Images/postlocked.gif' style='padding-right:5px;vertical-align:middle;'>";
				}
				echo"
				<tr>
					<td width='60%' class='forum-table-row' style='max-width:60%;padding:10px;'>
						<div class='topic-name-description' style='width:100%'>
							".$ForumImage."
							<a href='/forum/ShowPost.aspx?ID=".$Thread->ID."'>
								<font style='font-size:14px;font-weight:normal;color:#363636;vertical-align:middle;'>
									".$ThreadTitle."
								</font>
							</a>
...
<?
	...
	if ($Page > 1) {
	echo '<a href="/forum/ShowForum.aspx?ID='.$ID.'&Page='.($Page-1).'">Previous</a> - ';
	}
	echo '<b>Page '.$Page.' of '.(ceil($num / $Setting["PerPage"]));
	echo'</b>';
	if ($Page < ($amount)) {
	echo ' - <a href="/forum/ShowForum.aspx?ID='.$ID.'&Page='.($Page+1).'">Next</a>';
	}
?>
<?
