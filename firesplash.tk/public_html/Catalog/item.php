				...
				if(mysqli_num_rows($hatExists) > 0){
					...
					if(isset($_GET['remove']) && is_numeric($_GET['remove'])){
						...
						if(mysqli_num_rows($ourSale) > 0){
							...
							header("Location: ../Catalog/item.php?id=$Hat[0]"); exit;
						}else{
									...
									if(isset($_GET['buy'])){
									    if($_GET['buy']=='g'){
									        if($Hat['GOLD_PRICE']>0){
									            if($Hat['OFFSALE']!='YES'){
									                //not offsale
									                $ownedQ = mysqli_query($conn,"SELECT * FROM `ec_crate` WHERE `USER_ID` = '$user[ID]' AND `ITEM_ID` = '$Hat[ID]'");
									                if(mysqli_num_rows($ownedQ)==0){
									                    if($user['GOLD'] - $Hat['GOLD_PRICE'] >= 0){
									                        //can afford
    									                    if($Hat['RARE']=='YES'){
    									                        //It is limited
    									                        if($Hat['STOCK'] > 0){
        															...
        															mysqli_query($conn, "UPDATE `ec_items` SET `STOCK`='$NewStock' WHERE `ID`='$id'"); //update stock
        															mysqli_query($conn, "INSERT INTO `ec_crate` VALUES(NULL,'$id','$user[0]','$Serial')");//insert
        															$cash = $user['GOLD'] - $Hat['GOLD_PRICE'];
        															mysqli_query($conn, "UPDATE `ec_users` SET `GOLD`='$cash' WHERE `ID`='$user[0]'");//take gold
        															mysqli_query($conn, "INSERT INTO `ec_anti_horde` VALUES(NULL,'$id','$user[0]','1')");
        															echo"<script>window.alert('You Have Bought $Serial of $Hat[NAME] for $Hat[GOLD_PRICE] Gold! You now have $cash Gold Left!')</script>";
        															echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
    									                        }else{
    									                            //not in stock
    									                            echo"<script>window.alert('This item is out of stock!')</script>";
        															echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
    									                        }
    									                    }else{
    									                        //not limited
    									                        $cash = $user['GOLD'] - $Hat['GOLD_PRICE'];
    									                        //buy
    									                        mysqli_query($conn, "INSERT INTO `ec_crate` VALUES(NULL,'$id','$user[0]','0')");//insert
        															$cash = $user['GOLD'] - $Hat['GOLD_PRICE'];
        														mysqli_query($conn, "UPDATE `ec_users` SET `GOLD`='$cash' WHERE `ID`='$user[0]'");//take gold
        														echo"<script>window.alert('You Have Bought $Hat[NAME] for $Hat[GOLD_PRICE] Gold! You now have $cash Gold Left!')</script>";
        														echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
    									                    }
									                    }else{
									                        //cannot afford
									                        echo"<script>window.alert('You cannot afford this item!')</script>";
        													echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
									                    }
									                }else{
									                    //already owned
									                    echo"<script>window.alert('You already have this item!')</script>";
        												echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
									                }
									            }else{
									                //item offsale
									                echo"<script>window.alert('This item is offsale!')</script>";
        											echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
									            }
									        }else{
									            //item offsale
									        ...
									        if($Hat['SILVER_PRICE']>0){
									            if($Hat['OFFSALE']!='YES'){
									                ...
									                $ownedQ = mysqli_query($conn,"SELECT * FROM `ec_crate` WHERE `USER_ID` = '$user[ID]' AND `ITEM_ID` = '$Hat[ID]'");
									                if(mysqli_num_rows($ownedQ)==0){
									                    if($user['SILVER'] - $Hat['SILVER_PRICE'] >= 0){
									                        //can afford
    									                    if($Hat['RARE']=='YES'){
    									                        //It is limited
    									                        if($Hat['STOCK'] > 0){
        															...
        															mysqli_query($conn, "UPDATE `ec_items` SET `STOCK`='$NewStock' WHERE `ID`='$id'"); //update stock
        															mysqli_query($conn, "INSERT INTO `ec_crate` VALUES(NULL,'$id','$user[0]','$Serial')");//insert
        															$cash = $user['SILVER'] - $Hat['SILVER_PRICE'];
        															mysqli_query($conn, "UPDATE `ec_users` SET `SILVER`='$cash' WHERE `ID`='$user[0]'");//take gold
        															mysqli_query($conn, "INSERT INTO `ec_anti_horde` VALUES(NULL,'$id','$user[0]','1')");
        															echo"<script>window.alert('You Have Bought $Serial of $Hat[NAME] for $Hat[SILVER_PRICE] Silver! You now have $cash Silver Left!')</script>";
        															echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
    									                        }else{
    									                            //not in stock
    									                            echo"<script>window.alert('This item is out of stock!')</script>";
    									                        ...
    									                        mysqli_query($conn, "INSERT INTO `ec_crate` VALUES(NULL,'$id','$user[0]','0')");//insert
        															$cash = $user['SILVER'] - $Hat['SILVER_PRICE'];
        														mysqli_query($conn, "UPDATE `ec_users` SET `SILVER`='$cash' WHERE `ID`='$user[0]'");//take gold
        														echo"<script>window.alert('You Have Bought $Hat[NAME] for $Hat[SILVER_PRICE] Silver! You now have $cash Silver Left!')</script>";
        														echo"<script>window.location='/Catalog/item.php?id=$Hat[ID]'</script>";
    									                    }
									                    }else{
									                        //cannot afford
									...
									if(isset($_POST['comment'])){
										...
										if(mysqli_num_rows($hasComments) > 0){
											...
											if(isset($_GET['page']) && mysqli_num_rows($getComments) < 1){
												echo"<script>window.location='../Catalog/item.php?id=$id';</script>";
												exit;
											}
					...
					// bye!
					echo"<script>window.location='../Catalog';</script>"; exit;
				}
			}else{
				# bye!
				echo"<script>window.location='../Catalog';</script>"; exit;
