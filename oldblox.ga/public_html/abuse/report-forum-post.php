<?				
	...
	$CheckContent = mysql_query("SELECT * FROM Threads WHERE ID='".$ID."'");
	$Content = mysql_fetch_object($CheckContent);
	$Exist = mysql_num_rows($CheckContent);
	$getOffender = mysql_query("SELECT * FROM Users WHERE ID='$Content->PosterID'");
	...
	if($AbuseSubmit){
		if($User){
			if($AbuseCategory != "0"){
				...
				mysql_query("INSERT INTO Reports (UserWhoSent, UserReported, Content, Comment, Category, Type, ItemID) VALUES('$myU->ID','$gO->ID','Title: $TitleReported Body: $BodyReported','$AbuseComment','$AbuseCategory','ForumPost','$ID')");
